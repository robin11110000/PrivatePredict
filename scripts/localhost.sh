...
__SP_ROOT_DIR="${PrivatePredict_ROOT:-$(cd "${__SP_LOCAL_DIR}/.." && pwd)}"
...
# Guard: only run via ./PrivatePredict
...
init_env() {
  ...
  if grep -q '^BACKEND_IMAGE_NAME=' "${__SP_ROOT_DIR}/.env"; then
    "${SED_INPLACE[@]}" "s|^BACKEND_IMAGE_NAME=.*|BACKEND_IMAGE_NAME=ghcr.io/openpredictionmarkets/PrivatePredict-backend:latest|" "${__SP_ROOT_DIR}/.env"
  else
    printf "\nBACKEND_IMAGE_NAME=ghcr.io/openpredictionmarkets/PrivatePredict-backend:latest\n" >> "${__SP_ROOT_DIR}/.env"
  fi
  if grep -q '^FRONTEND_IMAGE_NAME=' "${__SP_ROOT_DIR}/.env"; then
    "${SED_INPLACE[@]}" "s|^FRONTEND_IMAGE_NAME=.*|FRONTEND_IMAGE_NAME=ghcr.io/openpredictionmarkets/PrivatePredict-frontend:latest|" "${__SP_ROOT_DIR}/.env"
  else
    printf "\nFRONTEND_IMAGE_NAME=ghcr.io/openpredictionmarkets/PrivatePredict-frontend:latest\n" >> "${__SP_ROOT_DIR}/.env"
  fi
