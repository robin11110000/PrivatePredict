...
# --------------------------------------------------------------------------------------
# Path resolution (stable even when this script is "sourced" by ./PrivatePredict)
# --------------------------------------------------------------------------------------
__SP_CMD_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
__SP_ROOT_DIR="${PrivatePredict_ROOT:-$(cd "${__SP_CMD_DIR}/.." && pwd)}"
...
# --------------------------------------------------------------------------------------
# Guard: must be run via ./PrivatePredict
# --------------------------------------------------------------------------------------
...
sp_up() {
  if [ "${APP_ENV}" = "production" ]; then
    ...
    docker network inspect PrivatePredict_external_network > /dev/null 2>&1 || \
      docker network create --driver bridge PrivatePredict_external_network
  ...
  if [ "${APP_ENV}" = "development" ]; then
    echo "PrivatePredict may be found at http://localhost:${FRONTEND_PORT} ."
