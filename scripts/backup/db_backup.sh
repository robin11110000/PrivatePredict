...
#
# Database backup & restore utilities for PrivatePredict
# Usage:
#   ./PrivatePredict backup --save
#   ./PrivatePredict backup --list
#   ./PrivatePredict backup --latest
#   ./PrivatePredict backup --restore </path/to/backup.dump.gz>
#   ./PrivatePredict backup --restore-latest
#   ./PrivatePredict backup --inspect </path/to/backup.dump.gz>
#   ./PrivatePredict backup --help
#
...
#
# Notes:
# - Must be called via ./PrivatePredict (enforced by CALLED_FROM_PrivatePredict guard)
# - Uses .env for DB container/name/user/pass/ports
# - Writes backups to a directory PARALLEL to repo root: ../backups/
# - Filenames: PrivatePredict_backup_${APP_ENV}_${YYYYmmdd_HHMMSS}.dump.gz + .sha256
...
# .env is already sourced by the PrivatePredict script before calling this file
...
need_container_running() {
  if ! container_running; then
    ...
    echo "Start it first: ./PrivatePredict up"
    exit 1
  fi
}
...
latest_backup_file() {
  ls -1t "$BACKUP_ROOT"/PrivatePredict_backup_"${APP_ENV}"_*.dump.gz 2>/dev/null | head -n 1 || true
}

print_usage() {
  cat <<EOF
Usage: ./PrivatePredict backup [--save | --list | --latest | --restore <file> | --restore-latest | --inspect <file> | --help]
...
Backups:
  PrivatePredict_backup_${APP_ENV}_YYYYmmdd_HHMMSS.dump.gz
  PrivatePredict_backup_${APP_ENV}_YYYYmmdd_HHMMSS.dump.gz.sha256
EOF
}
...
do_save() {
  ...
  ts="$(timestamp)"
  file="$BACKUP_ROOT/PrivatePredict_backup_${APP_ENV}_${ts}.dump.gz"
...
do_list() {
  ...
  ls -1t "$BACKUP_ROOT"/PrivatePredict_backup_"${APP_ENV}"_*.dump.gz 2>/dev/null || echo "(none found)"
}
