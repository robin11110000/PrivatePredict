...
* removed all compose scripts. They will be handled by the PrivatePredict script.
* removed all exec scripts. They will be handled by ./scripts/exec.sh script.
* removed start file.
* added script PrivatePredict that will handle all logic.
...
3. Make sure PrivatePredict file is executable
```
chmod +x PrivatePredict
```
...
5. Run PrivatePredict Script
```
sudo ./PrivatePredict install
```
...
* Database Name: the name for the database - Optional (defaults to PrivatePredict_db)
...
```
sudo ./PrivatePredict up
```

9. And stop it with:
```
sudo ./PrivatePredict down
```

## Logic Behind The Script:
* Each time you run the script (PrivatePredict) first of all it checks if docker compose is installed or not. It only supports docker compose and not docker-compose since docker-compose has stopped receiving security updates for about a year now.
...
* After that everything is ready to run with ./PrivatePredict up
...
* Add more options to the PrivatePredict script.
* Add a help section to the PrivatePredict script to offer help messages regarding its usage.
