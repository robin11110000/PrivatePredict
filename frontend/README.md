# PrivatePredict Frontend

This is the frontend application for PrivatePredict, built with React and Vite.
...
1. **Build the frontend Docker image:**
   ...
   ```bash
   cd /path/to/PrivatePredict
   cd frontend
   docker build -t PrivatePredict .
   ```

2. **Run the frontend container:**

   ```bash
   docker run -d -p 5173:5173 PrivatePredict
   ```
