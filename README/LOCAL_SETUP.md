...
For more information about the project go to: [PrivatePredict](https://github.com/openpredictionmarkets/PrivatePredict)
...
* You may also need to create a data file, which is in the `.gitignore`, as MacOS docker may not create this automatically upon installation. You might have to run the following commands from the main `./PrivatePredict` directory:
...
- The new user will have an initial account balance and maximum debt as configured in `setup.yaml`. **You can configure `setup.yaml` yourself, but must do so prior to installing PrivatePredict**.

### Shutting down the service:

- To shut down the service, navigate to the `./PrivatePredict` root directory in your terminal.
- Run `./PrivatePredict down`, which will use `docker compose down` to stop services.
...
```
docker exec -it -e PGPASSWORD=password PrivatePredict-postgres-container psql -U user -d PrivatePredict_db
```
...
```
docker exec PrivatePredict-postgres-container sh -c 'pg_dump -U user -F c PrivatePredict_db' > backup_before_resolution.dump 2> pgdump_error.log
```
...
docker exec -it PrivatePredict-postgres-container psql -U user -c "DROP DATABASE PrivatePredict_db;"
docker exec -it PrivatePredict-postgres-container psql -U user -c "CREATE DATABASE PrivatePredict_db;"

# Restore
cat backup_before_resolution.dump | docker exec -i PrivatePredict-postgres-container pg_restore --no-owner -U user -d PrivatePredict_db
```

### Setting Up a Staging Instance On the Web

While we don't like to say we have an official, "prod," version yet per se, we do give the ability for you to run an staging instance online which for all intents and purposes is a functioning web app. However we don't endorse this app as being completely secure and recoverable without extra work. We're working on getting PrivatePredict in a more reliable, secure state.
