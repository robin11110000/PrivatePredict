...
Update PrivatePredict’s frontend build system so that Vite’s server.allowedHosts and preview.allowedHosts are automatically populated from the .env file and deploy scripts, rather than being hard-coded. This ensures:
...
Test locally: run ./PrivatePredict dev and confirm Vite allows localhost + frontend.

Test prod: run ./PrivatePredict prod, verify .env populates VITE_PROD_ALLOWED_HOSTS, and that the frontend only accepts traffic from your configured domain(s).