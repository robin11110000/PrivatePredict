...
const environments = {
  ...
  staging: {
    API_BASE_URL: 'https://api-staging.PrivatePredict.com/api',
    APP_ENV: 'staging',
    DEBUG: true,
    ENABLE_DEVTOOLS: true,
    LOG_LEVEL: 'info',
  ...
  production: {
    API_BASE_URL: 'https://api.PrivatePredict.com/api',
    APP_ENV: 'production',
    DEBUG: false,
...
// .env.staging
REACT_APP_API_BASE_URL=https://api-staging.PrivatePredict.com/api
...
// .env.production
REACT_APP_API_BASE_URL=https://api.PrivatePredict.com/api
...
function getRuntimeCaching(mode) {
  ...
  return [
    {
      urlPattern: /^https:\/\/api\.PrivatePredict\.com\/api\/markets/,
...
jobs:
  deploy-staging:
    ...
    - name: Run smoke tests
      working-directory: ./frontend
      run: npm run test:smoke -- --baseURL=https://staging.PrivatePredict.com
  ...
  deploy-production:
    ...
    - name: Create deployment backup
      run: |
        ...
        aws s3 sync s3://${{ secrets.PRODUCTION_S3_BUCKET }} s3://PrivatePredict-backups/frontend/$timestamp/
    ...
    - name: Blue-Green Deployment
      run: |
        ...
        # Run health checks
        curl -f https://blue.PrivatePredict.com/health || exit 1
        ...
        # Final health check
        curl -f https://PrivatePredict.com/health || exit 1
        
    - name: Run production smoke tests
      working-directory: ./frontend
      run: npm run test:smoke -- --baseURL=https://PrivatePredict.com
  ...
  rollback:
    ...
    - name: Restore from backup
      run: |
        ...
        aws s3 sync s3://PrivatePredict-backups/frontend/${{ github.event.inputs.backup_timestamp }}/ s3://${{ secrets.PRODUCTION_S3_BUCKET }} --delete
...
terraform {
  ...
  backend "s3" {
    bucket = "PrivatePredict-terraform-state"
...
resource "aws_s3_bucket" "frontend" {
  bucket = "PrivatePredict-frontend-${var.environment}"
}
...
resource "aws_wafv2_web_acl" "frontend" {
  name  = "PrivatePredict-frontend-${var.environment}"
  scope = "CLOUDFRONT"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "PrivatePredict-frontend-${var.environment}"
    sampled_requests_enabled   = true
  }
}
