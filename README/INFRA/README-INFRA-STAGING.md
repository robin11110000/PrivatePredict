...
This document explains how PrivatePredict is deployed automatically to **staging** at [https://kconfs.com](https://kconfs.com), within the Open Prediction Markets Github organization.
...
- **Trigger:**  
  A **merge into `main`** on [openpredictionmarkets/PrivatePredict](https://github.com/openpredictionmarkets/PrivatePredict)  
...
1. A **Pull Request** is merged into `PrivatePredict@main`.
...
4. The `ansible_playbooks` repo has a workflow called `deploy_staging.yml` which:
   ...
   - Stops all running PrivatePredict containers.
   - Removes old images:
     - `PrivatePredict-staging-backend`
     - `PrivatePredict-staging-frontend`
   - Pulls the latest code from `openpredictionmarkets/PrivatePredict@main`.
...
jobs:
  deploy:
    name: Deploy PrivatePredict to Staging
