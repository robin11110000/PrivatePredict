...
The following is a general tutorial geared at getting [PrivatePredict](https://github.com/openpredictionmarkets/PrivatePredict) set up on Digital Ocean. Other virtual machine servers may work similarly, this serves as an example and is designed for more novice users.
...
We are using Digital Ocean as a server, since it's predictable, easy and cheap. PrivatePredict is designed to be performant, meaning it can perform well on a small server.
...
### Clone the PrivatePredict Repo

Now that the domain has been pointed in the right direction, you can clone the PrivatePredict repo. First, check what version of `git` you are running by typing the following in the console:
...
```
root@DROPLET_NAME:/home# git clone https://github.com/openpredictionmarkets/PrivatePredict.git
```
From here, navigate to your `PrivatePredict` folder:

```
root@DROPLET_NAME: cd /home/PrivatePredict
```
And run `./PrivatePredict install`:

```
root@DROPLET_NAME:/home/PrivatePredict ./PrivatePredict install
```

PrivatePredict will prompt you to type `1` for development, `2` for production, or `3` to quit. Hit `2` on your keyboard to start.

![Screenshot 2024-08-03 111737](https://github.com/user-attachments/assets/3423a047-213b-48cc-baf3-4eadb326c111)

Next, PrivatePredict will prompt you for the name of your domain.
...
Once you're done, type `./PrivatePredict up` to spin up a PrivatePredict instance ready to deploy to the web, and navigate to your domain to see if it works.

If you want to spin down your PrivatePredict instance, just type `./PrivatePredict down` in your console. **If you need to reinstall PrivatePredict, make sure to run `./PrivatePredict down` first**.

#### Using PrivatePredict on the web

Congratulations, you should now have a version of PrivatePredict deployed to the web! To get started with it, log in with your admin credentials. From here, you will not be able to create markets, but you will be able to create new users who do not have admin powers but can create markets.
...
By default users start with an initial account balance of 0 credits and can go up to 500 credits in debt. This can be changed by editing `setup.yaml` **prior to installing PrivatePredict**.
...
```
docker exec -it -e PGPASSWORD=${POSTGRES_PASSWORD} PrivatePredict-postgres-container psql -U ${POSTGRES_USER} -d PrivatePredict_db
```
...
-- `git clone https://github.com/openpredictionmarkets/PrivatePredict.git`
...
- If you have not run this previously, within the root of the `./PrivatePredict` directory, create a `.env` file which will be ignored in the gitignore.
- Within the root of the `./PrivatePredict` repo, run the command, `./PrivatePredict install`.
...
- When this has completed, run `./PrivatePredict up`...this will use `docker compose` to spin up the images into containers on your machine.
